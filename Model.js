...
// ---- Cronos task helpers. Date keys are the "yyyy-MM-dd" stamps the cell
//      grid above uses, so a day either has tasks or it does not.

function openTasks(tasks) {
  return Array.isArray(tasks) ? tasks.filter(function(t) { return t && t.done !== true }) : []
}

function dueKey(task) {
  var due = task && typeof task.due === "string" ? task.due : ""
  return due.length >= 10 ? due.slice(0, 10) : ""
}

// Map of date key to the list of open tasks due that day.
function taskDayMap(tasks) {
  var out = {}
  var open = openTasks(tasks)
  for (var i = 0; i < open.length; i++) {
    var key = dueKey(open[i])
    if (key === "") continue
    if (!out[key]) out[key] = []
    out[key].push(open[i])
  }
...
// Open tasks with a due date, ascending by date, capped to `limit`.
function upcomingTasks(tasks, limit) {
  var open = openTasks(tasks).filter(function(t) { return dueKey(t) !== "" })
  open.sort(function(a, b) {
    return dueKey(a) < dueKey(b) ? -1 : (dueKey(a) > dueKey(b) ? 1 : 0)
  })
  return open.slice(0, limit === undefined ? 10 : limit)
}
